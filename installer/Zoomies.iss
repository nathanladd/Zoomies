; Inno Setup script for Zoomies
; -----------------------------------------------------------------------------
; Build the PyInstaller bundle first (pyinstaller Zoomies.spec), then compile
; this script with:
;   ISCC.exe /DAppVersion=0.2.0 installer\Zoomies.iss
...
#define AppName         "Zoomies"
#define AppDisplayName  "Zoomies"
#define AppPublisher    "Nathan Ladd"
#define AppURL          "https://github.com/nathanladd/Zoomies"
#define AppExeName      "Zoomies.exe"
...
OutputBaseFilename=Zoomies-Setup-{#AppVersion}
#if FileExists(AddBackslash(SourcePath) + "Zoomies_App_Icon.ico")
SetupIconFile={#SourcePath}\Zoomies_App_Icon.ico
#endif
...
; If Zoomies.exe is running during install/uninstall, Inno Setup checks for
...
AppMutex=ZoomiesSingletonMutex
...
[Files]
; Entire PyInstaller one-dir bundle (dist/Zoomies/*) goes into {app}.
Source: "..\dist\Zoomies\*"; DestDir: "{app}"; Flags: recursesubdirs createallsubdirs ignoreversion
...
[UninstallDelete]
; %LOCALAPPDATA%\Zoomies holds the DB, uploaded media, and backups. Leave
...
begin
  if CurUninstallStep = usPostUninstall then begin
    UserData := ExpandConstant('{localappdata}\Zoomies');
    if DirExists(UserData) then begin
      if MsgBox(
            'Also delete Zoomies user data (question database, uploaded images, backups)?' + Chr(13) + Chr(10) +
            Chr(13) + Chr(10) +
